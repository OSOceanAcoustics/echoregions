...
class Region2DParser(EvParserBase):
    ...
    def __init__(self, input_file=None):
        ...
        self.raw_range = None
        self.min_depth = (
            None  # Set to replace -9999.9900000000 range values which are EVR min range
        )
        self.max_depth = (
            None  # Set to replace 9999.9900000000 range values which are EVR max range
        )

    def _parse(self, fid, convert_time=False, convert_depth_edges=False, offset=0):
        ...
        def _region_metadata_to_dict(line):
            ...
            top_y = self.swap_range_edge(line[9]) if convert_depth_edges else line[9]
            bottom_y = (
                self.swap_range_edge(line[12]) if convert_depth_edges else line[12]
            )
            top_y = float(top_y) + offset
            bottom_y = float(bottom_y) + offset

            left_x = (
                parse_time(f"D{line[7]}T{line[8]}")
                if convert_time
                else f"D{line[7]}T{line[8]}"
            )
            right_x = (
                parse_time(f"D{line[10]}T{line[11]}")
                if convert_time
                else f"D{line[10]}T{line[11]}"
            )
            return {
                ...
                "bounding_rectangle_calculated": line[
                    6
    ...
    def to_dataframe(self, **kwargs):
        ...
        metadata = pd.Series(self.data["metadata"])
        row = []
        # Loop over each region
        for rid, region in self.data["regions"].items():
            # Save region information for each point
            region_metadata = pd.Series(region["metadata"])
            region_notes = pd.Series({"notes": region["notes"]})
            detection_settings = pd.Series(
                {"detection_settings": region["detection_settings"]}
            )
    ...
    def set_range_edge_from_raw(self, raw, model="EK60"):
        try:
            ...
            raise ModuleNotFoundError(
                "This function requires 'echopype' to be installed"
            ) from e
    ...
    def convert_output(self, convert_time=True, convert_depth_edges=True):
        for region in self.data["regions"].values():
            if convert_time:
                region["metadata"]["bounding_rectangle_left_x"] = parse_time(
                    region["metadata"]["bounding_rectangle_left_x"]
                )
                region["metadata"]["bounding_rectangle_right_x"] = parse_time(
                    region["metadata"]["bounding_rectangle_left_x"]
                )
            if convert_depth_edges:
                region["metadata"]["bounding_rectangle_top_y"] = self.swap_range_edge(
                    region["metadata"]["bounding_rectangle_top_y"]
                )
                region["metadata"][
                    "bounding_rectangle_bottom_y"
                ] = self.swap_range_edge(
                    region["metadata"]["bounding_rectangle_bottom_y"]
                )
            region["points"] = self.convert_points(
                region["points"], convert_time, convert_depth_edges
            )
